...
<div>
  ...
  <div style="margin: 10px 0">
    ...
    <el-upload :action="'http://' + serverIp + ':9090/employee/import'" :show-file-list="false" accept="xlsx"
               :on-success="handleExcelImportSuccess" :on-error="handleExcelImportError" style="display: inline-block">
...
<!--    <el-table-column prop="id" label="ID" width="80"></el-table-column>-->
    ...
    <el-table-column prop="role" label="角色" width="100">
      <template slot-scope="scope">
        ...
        <el-tag type="warning" v-if="scope.row.role === 'ROLE_TEACHER'">培训老师</el-tag>
        <el-tag type="success" v-if="scope.row.role === 'ROLE_STUDENT'">学生</el-tag>
        <el-tag  v-if="scope.row.role === 'ROLE_MANAGER'">培训管理员</el-tag>
      </template>
    </el-table-column>
...
export default {
  ...
  methods:{
    ...
    //导出
    exp() {
      window.open(`http://${serverIp}:9090/employee/export`)
    },
    ...
    handleExcelImportError() {
      this.$message.error("导入失败，仅支持xlsx类型表格")
      this.load()
    }
  }
}
