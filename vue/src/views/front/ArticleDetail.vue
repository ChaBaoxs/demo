<template>
  <div style="color: #666;margin:20px 100px 50px 100px">
    <h1 style="float: left;margin-right: 15px;color: #5392a6">知识专区:</h1>
    <div>
      <h1 class="article" @click="article1()">文章讨论区</h1>
            ...
            <div style="flex: 1; font-size: 14px; padding: 5px 0; line-height: 25px">
              ...
              <div style="display: flex; line-height: 20px; margin-top: 5px">
                ...
                <div style="text-align: right; flex: 1">
                  ...
                  <el-button type="text" style="color: red" @click="del(item.id)" v-if="user.role === 'ROLE_ADMIN' || user.id === item.userId">删除</el-button>
                </div>
              </div>
            </div>   <!--  内容-->
          </div>
          ...
          <div v-if="item.children.length"  style="padding-left: 200px;">
            <div v-for="subItem in item.children" :key="subItem.id"  style="background-color: #f0f0f0; padding: 5px 20px">
              ...
              <div style="font-size: 14px; padding: 5px 0; line-height: 25px">
                <div>
                  <b style="color: #3a8ee6" v-if="subItem.pname">回复：{{ subItem.pname }}</b>
                </div>
...
export default {
  ...
  methods: {
    ...
    save() {
      ...
      //   return
      // }
      this.commentForm.articleId = this.id//保存到对应文章评论区
